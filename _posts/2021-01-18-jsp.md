...
| 단점 | 서비스가 한정적이고 관리가 힘들다(추가/수정/삭제 모두 수동) | 상대적으로 느리고 추가 비용이 든다(서버 비용 혹은 웹페이지가 느려지는 부분) |
...
Implicit Objects는 한국어로 암시적/묵시적 객체라는 뜻으로 자바 객체이다. 암시적 객체라는 뜻은 개발자가 명시적으로 선언하지 않아도 JSP 컨테이너에서 제공하는 객체라는 뜻이다. 그래서 이 암시적 객체를 Pre-defined object(사전정의 객체)라고 표현하기도 한다. 

| **암시적 객체** | **설명**                                                |
| --------------- | ------------------------------------------------------- |
| request         | 요청과 관련된 HttpServletRequest 객체                   |
| response        | 클라이언트에 대한 응답과 연관된 HttpServletRequest 객체 |
| out             | 클라이언트에 출력을 보내는 데 사용되는 PrintWriter 객체 |
| session         | 요청과 관련된 HttpServletRequest 객체                   |
| application     | Application Context와 연관된 ServletContext 객체        |
| config          | 페이지와 연관된 ServletConfig 객체                      |
...
*javax.servlet.http.HttpServletRequest* 또는 *javax.servlet.HttpServletRequest* 타입의 암시적 객체로 웹 컨테이너에 의해 만들어지고 각 파라미터를 포함한 각 JSP 요청 정보를 담고 있는 객체이다.
...
*javax.servlet.http.HttpServletResponse* 또는 *javax.servlet.HttpServletResponse* 타입의 암시적 객체로 웹 컨테이너에 의해 만들어지고 각 JSP 요청에 대한 응답 정보를 담고 있는 객체이다.
